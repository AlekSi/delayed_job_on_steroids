...
Run @script/generate delayed_job@ to add @script/delayed_job@. This script can then be used to manage a process which will start working off jobs.
  ...
  $ ruby script/delayed_job
  $ ruby script/delayed_job --job-types=SimpleJob    # to run only specific types of jobs
...
This fork was born to introduce new features to delayed_job, but also to be fully comatible with it.


h3. Incompatibility with tobi's delayed_job

This gem is designed to be fully compatible with tobi's original code. The only thing your should change is datebase schema: last_error column's type changed from string to text.


h3. Changes

* 2.0.0: TODO: Added script to run as daemon and control it.
         Timeout for job execution.
         Added send_at method for queueing jobs in the future.

* 1.7.5: Added possibility to run only specific types of jobs.
